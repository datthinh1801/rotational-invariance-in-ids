...
def parse_cli_arguments():
    ...
    parser.add_argument(
        ...
        default="MLP",
        type=str,
        choices=["MLP", "Noemb", "pos_singleMLP"],
    )
    ...
    parser.add_argument(
        "--optimizer", default="AdamW", type=str, choices=["AdamW", "Adam", "SGD"]
    )
    parser.add_argument(
        "--scheduler", default="cosine", type=str, choices=["cosine", "linear"]
    )
    ...
    parser.add_argument(
        "--pt_tasks",
        default=["contrastive", "denoising"],
        type=str,
        nargs="*",
        choices=["contrastive", "contrastive_sim", "denoising"],
    )
    parser.add_argument(
        "--pt_aug", default=[], type=str, nargs="*", choices=["mixup", "cutmix"]
    )
    ...
    parser.add_argument(
        "--final_mlp_style", default="sep", type=str, choices=["common", "sep"]
    )
...
if __name__ == "__main__":
    ...
    modelsave_path = os.path.join(
        os.getcwd(), opt.savemodelroot, opt.task, str(opt.dset_id), opt.run_name
    )
    ...
    (
        ...
        opt.dset_id, opt.dset_seed, opt.task, datasplit=[0.65, 0.15, 0.2]
    )
    ...
    trainloader = DataLoader(
        train_ds, batch_size=opt.batchsize, shuffle=True, num_workers=4
    )

    valid_ds = DataSetCatCon(X_valid, y_valid, cat_idxs, opt.dtask, continuous_mean_std)
    validloader = DataLoader(
        valid_ds, batch_size=opt.batchsize, shuffle=False, num_workers=4
    )

    test_ds = DataSetCatCon(X_test, y_test, cat_idxs, opt.dtask, continuous_mean_std)
    testloader = DataLoader(
        test_ds, batch_size=opt.batchsize, shuffle=False, num_workers=4
    )
    if opt.task == "regression":
        y_dim = 1
    else:
    ...
    if opt.pretrain:
        ...
        model = SAINT_pretrain(
            model, cat_idxs, X_train, y_train, continuous_mean_std, opt, device
        )
    ...
    for epoch in range(opt.epochs):
        ...
        if opt.active_log:
            wandb.log(
                {"epoch": epoch, "train_epoch_loss": running_loss, "loss": loss.item()}
            )
        if epoch % 5 == 0:
            model.eval()
            with torch.no_grad():
                if opt.task in ["binary", "multiclass"]:
                    accuracy, auroc = classification_scores(
                        model, validloader, device, opt.task, vision_dset
                    )
                    test_accuracy, test_auroc = classification_scores(
                        model, testloader, device, opt.task, vision_dset
                    )

                    print(
                        "[EPOCH %d] VALID ACCURACY: %.3f, VALID AUROC: %.3f"
                        % (epoch + 1, accuracy, auroc)
                    )
                    print(
                        "[EPOCH %d] TEST ACCURACY: %.3f, TEST AUROC: %.3f"
                        % (epoch + 1, test_accuracy, test_auroc)
                    )
                    if opt.active_log:
                        wandb.log({"valid_accuracy": accuracy, "valid_auroc": auroc})
                        wandb.log(
                            {"test_accuracy": test_accuracy, "test_auroc": test_auroc}
                        )
                    if opt.task == "multiclass":
                        if accuracy > best_valid_accuracy:
                            ...
                            torch.save(
                                model.state_dict(),
                                "%s/bestmodel.pth" % (modelsave_path),
                            )
                    else:
                        if accuracy > best_valid_accuracy:
                            best_valid_accuracy = accuracy
                    ...
                    if valid_rmse < best_valid_rmse:
                        ...
                        torch.save(
                            model.state_dict(), "%s/bestmodel.pth" % (modelsave_path)
                        )
            model.train()
